...
- item_id (UUID, FK → marketplace_items.id)
- quantity (INTEGER)
- status (ENUM)
- created_at (TIMESTAMP)
- updated_at (TIMESTAMP)
...
1. profiles (1) → (N) Farm (owner_id)
2. profiles (1) → (N) MarketplaceItem (seller_id)
3. profiles (1) → (N) Order (buyer_id)
...
function generatePostgresSchema() {
  return `-- Postgres Schema for Elghella Agritech Platform
...
-- Profiles table (already exists, but here's the schema)
...
export const CreateProfileSchema = z.object({
  ...
  user_type: z.enum(['farmer', 'buyer']).default('farmer'),
