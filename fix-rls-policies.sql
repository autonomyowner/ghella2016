-- Fix RLS Policies for all marketplaces
-- This script enables proper RLS policies for authenticated users
...
-- Create policies for categories (read-only for all)
...
SELECT 
    ...
    with_check
FROM pg_policies 
WHERE tablename IN ('equipment', 'animal_listings', 'land_listings', 'nurseries', 'vegetables', 'profiles', 'categories')
