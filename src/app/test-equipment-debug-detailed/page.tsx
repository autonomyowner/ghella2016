'use client';

import React, { useState } from 'react';
import { useSupabaseAuth } from '@/contexts/SupabaseAuthContext';
import { useEquipment } from '@/hooks/useSupabase';
import { supabase } from '@/lib/supabase/supabaseClient';

const TestEquipmentDebugDetailedPage: React.FC = () => {
  const { user } = useSupabaseAuth();
  const { equipment, loading, error, fetchEquipment } = useEquipment();
  const [results, setResults] = useState<string[]>([]);

  const addResult = (message: string) => {
    setResults(prev => [...prev, `${new Date().toLocaleTimeString()}: ${message}`]);
  };

  const testStepByStep = async () => {
    addResult('=== DETAILED DEBUG TEST ===');
    addResult(`User: ${user ? user.email : 'Not logged in'}`);
    addResult(`User ID: ${user?.id || 'None'}`);
    addResult(`Current equipment count: ${equipment.length}`);
    addResult(`Loading: ${loading}`);
    addResult(`Error: ${error || 'None'}`);
    
    try {
      addResult('Step 1: Testing direct Supabase query...');
      
      const { data, error: supabaseError } = await supabase
        .from('equipment')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(50);

      if (supabaseError) {
        addResult(`❌ Direct query failed: ${supabaseError.message}`);
        addResult(`Error code: ${supabaseError.code}`);
      } else {
        addResult(`✅ Direct query successful: ${data?.length || 0} items`);
        if (data && data.length > 0) {
          data.slice(0, 3).forEach((item: any, index: number) => {
            addResult(`  ${index + 1}. ${item.title} (User: ${item.user_id})`);
          });
        }
      }

      addResult('Step 2: Testing hook fetchEquipment...');
      addResult('About to call fetchEquipment()...');
      
      const result = await fetchEquipment();
      
      addResult(`✅ fetchEquipment completed!`);
      addResult(`📦 Function returned: ${Array.isArray(result) ? result.length : 0} items`);
      addResult(`📊 Equipment state after: ${equipment.length} items`);
      
      if (Array.isArray(result) && result.length > 0) {
        addResult('Function returned data:');
        result.slice(0, 3).forEach((item: any, index: number) => {
          addResult(`  ${index + 1}. ${item.title}`);
        });
      }
      
    } catch (err) {
      addResult(`❌ Error: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
    
    addResult('=== DEBUG COMPLETE ===');
  };

  const clearResults = () => {
    setResults([]);
  };

  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1>🔍 Detailed Debug Test</h1>
      
      <div style={{ 
        padding: '10px', 
        backgroundColor: '#fff3cd', 
        border: '1px solid #ffeaa7',
        borderRadius: '4px',
        marginBottom: '20px'
      }}>
        <strong>Instructions:</strong>
        <ol style={{ margin: '5px 0', paddingLeft: '20px' }}>
          <li>Open browser console (F12)</li>
          <li>Click "Run Detailed Debug"</li>
          <li>Watch both this page and console</li>
          <li>This will test both direct query and hook function</li>
        </ol>
      </div>

      <div style={{ marginBottom: '20px' }}>
        <button 
          onClick={testStepByStep}
          disabled={loading}
          style={{
            padding: '10px 20px',
            backgroundColor: loading ? '#ccc' : '#007bff',
            color: 'white',
            border: 'none',
            borderRadius: '4px',
            cursor: loading ? 'not-allowed' : 'pointer',
            marginRight: '10px'
          }}
        >
          {loading ? 'Loading...' : 'Run Detailed Debug'}
        </button>

        <button 
          onClick={clearResults}
          style={{
            padding: '10px 20px',
            backgroundColor: '#6c757d',
            color: 'white',
            border: 'none',
            borderRadius: '4px',
            cursor: 'pointer'
          }}
        >
          Clear Results
        </button>
      </div>

      <div style={{ 
        padding: '10px', 
        backgroundColor: '#f8f9fa', 
        border: '1px solid #dee2e6',
        borderRadius: '4px',
        maxHeight: '400px',
        overflowY: 'auto',
        fontFamily: 'monospace',
        fontSize: '12px'
      }}>
        {results.length === 0 ? (
          <div style={{ color: '#6c757d' }}>Click "Run Detailed Debug" to start...</div>
        ) : (
          results.map((result, index) => (
            <div key={index} style={{ marginBottom: '5px' }}>
              {result}
            </div>
          ))
        )}
      </div>

      <div style={{ 
        padding: '10px', 
        backgroundColor: '#e9ecef', 
        border: '1px solid #dee2e6',
        borderRadius: '4px',
        marginTop: '20px'
      }}>
        <strong>Current Status:</strong>
        <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
          <li>Loading: {loading ? 'Yes' : 'No'}</li>
          <li>Equipment Count: {equipment.length}</li>
          <li>Error: {error || 'None'}</li>
          <li>User: {user ? user.email : 'Not logged in'}</li>
        </ul>
      </div>
    </div>
  );
};

export default TestEquipmentDebugDetailedPage; 